...
> 本分支为 **[OpenCode Go](https://opencode.ai)** 提供原生支持。OpenCode Go 是一个高性价比的第三方供应商，精选了多个优质模型。
...
- **动态格式路由** — MiniMax 系列模型（`minimax-*`）自动使用 Anthropic Messages API，其余模型使用 OpenAI Chat API。
- **自动剥离 cache_control** — 自动移除 Anthropic `cache_control` 字段以确保上游兼容。
- **精选默认模型** — `kimi-k2.6`（Sonnet）、`deepseek-v4-flash`（Haiku）、`glm-5.1`（Opus）。
- **通用配置** — 一份供应商配置可在 OpenCode 和 OpenClaw 之间共享。
