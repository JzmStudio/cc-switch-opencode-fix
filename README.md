...
> This fork adds first-class support for **[OpenCode Go](https://opencode.ai)**, an affordable third-party provider with a curated model lineup.
...
- **Dynamic format routing** — MiniMax series models (`minimax-*`) automatically use Anthropic Messages API, while all other models use OpenAI Chat API.
- **Cache-control stripping** — Automatically removes Anthropic `cache_control` fields for upstream compatibility.
- **Curated model defaults** — `kimi-k2.6` (Sonnet), `deepseek-v4-flash` (Haiku), `glm-5.1` (Opus).
- **Universal config** — One provider config can be shared across OpenCode and OpenClaw.
